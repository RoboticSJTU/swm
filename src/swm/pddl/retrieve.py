...
def _clean_action_comment(s: str) -> str:
    ...
    s = re.sub(
        r"\bwith\s+(the\s+)?(right\s+|left\s+)?hand\b", "", s, flags=re.IGNORECASE
    )
...
def _comment_before(text: str, idx: int) -> tuple[str, int]:
    ...
    if (
        ...
        and not re.match(r"\s*;;\s*source\s*:", prev_line, flags=re.IGNORECASE)
    ):
...
class Qwen3RerankerLite:
    ...
    @torch.no_grad()
    def score_docs(
        self, instruct: str, query: str, docs: list[str], batch_size: int = 16
    ) -> torch.Tensor:
        """
        返回每个 doc 的 p(yes)
        """
...
def build_pddl_action_template(
    ...
    with _MODEL_LOCK:
        for step in steps:
            scores = rr.score_docs(
                query_instruct, step, docs, batch_size=batch_size
            )  # [N]
            ...
            for sc, i in zip(vals.tolist(), idx.tolist()):
                op = op_names[i]
                top.append(
                    {"operator": op, "comment": mapping.get(op, ""), "score": float(sc)}
                )
                selected_ops.add(op)

            retrieval_log.append({"step": step, "retrieve": top})

    out_log.write_text(
        json.dumps(retrieval_log, ensure_ascii=False, indent=2), encoding="utf-8"
    )
