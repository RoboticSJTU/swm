...
prompt_path = Path("/home/xyx/下载/swm/src/swm/prompt_templates/vlm_cot.txt")
...
def process_task(task_info):
    ...
    raw_output_file = save_dir / "raw_output.txt"
    nl_plan_path = save_dir / "plan_nl.txt"
    plan_path = save_dir / "plan.txt"

    if nl_plan_path.exists():
    ...
    try:
        ...
        raw_output_file.write_text(output, encoding="utf-8")
        ...
        try:
            ...
            if not isinstance(plan_sequence, list):
                raise ValueError("plan_sequence is not a list")

            plan_lines = [str(x).strip() for x in plan_sequence if str(x).strip()]
            if not plan_lines:
                return task_info, "failed", "empty_plan"
        ...
        plan_text = "\n".join(plan_lines)
        nl_plan_path.write_text(plan_text, encoding="utf-8")
        plan_path.write_text(plan_text, encoding="utf-8")  # 兼容保留
...
if __name__ == "__main__":
    ...
    for task_domain, items in instructions.items():
        ...
        for task_name, instruction in items.items():
            ...
            image_path = next(
                (
                    task_dir / f"task_{task_index}.{ext}"
                    for ext in ("jpg", "png", "jpeg", "webp")
                    if (task_dir / f"task_{task_index}.{ext}").exists()
                ),
                None,
            )
    ...
    print("\n" + "=" * 80)
    print(f"开始进行 Judge 评测，共 {len(generated_tasks)} 个任务")
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as ex:
        futures = {ex.submit(process_judge, t): t for t in generated_tasks}
        for i, fut in enumerate(as_completed(futures), 1):
            task_info, status, info = fut.result()
            task_domain, task_index = task_info[:2]
            print(f"\n[Judge {i}/{len(generated_tasks)}] {task_domain}-{task_index}")
    ...
    gen_rate = success / total_tasks * 100 if total_tasks else 0
    ...
    report_lines = [
        ...
        "📊 评估完成统计报告",
        f"📋 总任务数: {total_tasks}",
        f"✅ 动作序列生成成功数: {success}, 成功率 {gen_rate:.1f}%",
        f"❌ JSON解析失败: {json_parse_fail}",
        f"⚠️ 动作序列为空: {empty_plan_fail}",
        f"❌ 其他失败: {other_fail}",
